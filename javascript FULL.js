 ...
 * Versão: 3.8 - Correção do scroll infinito de vídeos
 * ============================================================
 */
...
function mudarAba(aba) {
    ...
    const isChangingFromVideos = secaoVideos && secaoVideos.style.display !== 'none' && aba !== 'videos';
    if (isChangingFromVideos) {
    ...
    if (aba === 'videos') {
        ...
        if (secaoVideos) {
            ...
            // 🔥 CORREÇÃO: Recria o sentinela se ele não existir
            const sentinela = document.getElementById('scroll-sentinel-videos');
            if (!sentinela) {
                configurarScrollInfinitoVideos();
            } else if (!sentinelaObserver) {
                // Se o sentinela existe mas o observer foi desconectado
                sentinelaObserver = new IntersectionObserver((entries) => {
                    entries.forEach(entry => {
                        if (entry.isIntersecting && !estaCarregandoVideos && 
                            videosCarregados < todosOsVideos.length && secaoVideosAtiva) {
...
// ============================================================
// 22. FEED - CARREGAR VÍDEOS DO JSON (SCROLL INFINITO)
// ============================================================
let todosOsVideos = [];
 ...
 * 🔥 CORRIGIDO: Mantém o sentinela consistente e evita duplicação
 */
function carregarProximosVideos() {
    ...
    console.log('  - Vídeos carregados:', videosCarregados);
    console.log('  - Total de vídeos:', todosOsVideos.length);
    ...
    if (videosCarregados >= todosOsVideos.length) {
        ...
        if (sentinela) {
            ...
            sentinela.style.color = '#2ecc71';
        }
        return;
    }

    // Previne carregamento duplicado
    if (estaCarregandoVideos) {
    ...
    console.log('📦 Lote atual:', proximos.length, 'vídeos (', videosCarregados, 'a', videosCarregados + proximos.length - 1, ')');

    // Cria os cards
    proximos.forEach((video, index) => {
        console.log(`  ➕ Adicionando vídeo ${videosCarregados + index + 1}: ${video.title}`);
        const card = criarCardVideo(video);
        container.appendChild(card);
    });

    videosCarregados += proximos.length;
    console.log('✅ Lote carregado! Total:', videosCarregados, 'de', todosOsVideos.length);
    ...
    // 🔥 CORREÇÃO: Move o sentinela para o final após adicionar novos vídeos
    const sentinela = document.getElementById('scroll-sentinel-videos');
    if (sentinela) {
        container.appendChild(sentinela); // Move para o final
        if (videosCarregados < todosOsVideos.length) {
 ...
 * 🔥 CORRIGIDO: Garante que o sentinela seja criado e observado corretamente
 */
function configurarScrollInfinitoVideos() {
    ...
    // 🔥 CRIA O SENTINELA LOGO APÓS OS CARDS EXISTENTES
    // Ele será adicionado no final do container, mas antes de qualquer outro elemento
    ...
    sentinelaObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting && !estaCarregandoVideos && 
                videosCarregados < todosOsVideos.length && secaoVideosAtiva) {
                console.log('🔄 Carregando mais vídeos... (sentinela visível)');
                // 🔥 CORREÇÃO: Chama diretamente com um pequeno delay para evitar múltiplos disparos
                setTimeout(() => {
                    carregarProximosVideos();
                }, 100);
            }
 ...
 * 🔥 CORRIGIDO: Recria o sentinela e reconecta o observer
 */
function limparVideosCarregados() {
    ...
    if (cards.length > 21) {
        ...
        // 🔥 CORREÇÃO: Remove e recria o sentinela para garantir que funciona
        const sentinelaAntigo = document.getElementById('scroll-sentinel-videos');
        if (sentinelaAntigo) sentinelaAntigo.remove();
        
        // 🔥 CORREÇÃO: Desconecta o observer antigo
        if (sentinelaObserver) {
            sentinelaObserver.disconnect();
            sentinelaObserver = null;
        }
        
        // 🔥 CORREÇÃO: Recria o sentinela e o observer
        configurarScrollInfinitoVideos();
 ...
 * 🔥 CORRIGIDO: Reconecta o observer corretamente ao voltar
 */
function configurarCleanupAoMudarAba() {
    ...
    secaoVideosAtiva = secaoVideos.style.display !== 'none';
    ...
    abaObserver = new MutationObserver(() => {
        ...
        if (!isActive && secaoVideosAtiva) {
            ...
            // 🔥 CORREÇÃO: Garante que o sentinela existe e está sendo observado
            const sentinela = document.getElementById('scroll-sentinel-videos');
            if (!sentinela) {
                configurarScrollInfinitoVideos();
            } else if (!sentinelaObserver) {
                ...
                sentinelaObserver.observe(sentinela);
                console.log('✅ Observer reconectado ao sentinela existente.');
            }
        }
    });
...
// ============================================================
// 26. FEED - SISTEMA DE ARTIGOS
// ============================================================
let todosArtigos = [];
let artigosCarregados = false;
let artigosExibidos = 0;
let artigosSentinelaObserver = null;
let artigosSentinela = null;
...
async function carregarTodosArtigos() {
    ...
    todosArtigos = [];
    if (artigosSentinelaObserver) {
        artigosSentinelaObserver.disconnect();
        artigosSentinelaObserver = null;
    }
...
function carregarMaisArtigos() {
    ...
    if (artigosSentinela) {
        container.appendChild(artigosSentinela);
    }
    if (artigosExibidos >= todosArtigos.length) {
        if (loading) {
            loading.style.display = 'none';
            if (artigosSentinela) {
                artigosSentinela.textContent = '🎉 Todos os artigos foram carregados!';
                artigosSentinela.style.color = 'var(--primary-purple)';
            }
        }
        if (artigosSentinelaObserver) {
            artigosSentinelaObserver.disconnect();
...
function configurarScrollInfinitoArtigos() {
    ...
    const loading = document.getElementById('artigos-loading');
    if (artigosSentinela) {
        artigosSentinela.remove();
        artigosSentinela = null;
    }
    const sentinela = document.createElement('div');
    sentinela.id = 'artigos-scroll-sentinel';
    sentinela.style.height = '20px';
    sentinela.style.width = '100%';
    sentinela.style.visibility = 'visible';
    sentinela.style.display = 'flex';
    sentinela.style.justifyContent = 'center';
    sentinela.style.alignItems = 'center';
    sentinela.style.color = 'var(--text-dim)';
    ...
    sentinela.textContent = '🔄 Carregando mais artigos...';
    container.appendChild(sentinela);
    artigosSentinela = sentinela;
    if (artigosSentinelaObserver) {
        artigosSentinelaObserver.disconnect();
        artigosSentinelaObserver = null;
    }
    artigosSentinelaObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && artigosCarregados && artigosExibidos < todosArtigos.length) {
                if (loading) {
                    loading.style.display = 'block';
                }
                carregarMaisArtigos();
    ...
    artigosSentinelaObserver.observe(sentinela);
}
...
// ============================================================
// 27. ARTIGOS - MODAL EM TELA CHEIA
// ============================================================
