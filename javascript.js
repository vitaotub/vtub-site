...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); deferredPrompt = e; pwaPopup.style.display = 'flex'; });
    const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone;
    if (!isStandalone) { setTimeout(() => { if (pwaPopup.style.display !== 'flex') pwaPopup.style.display = 'flex'; }, 2000); }
    ...
    if (installBtn) {
        installBtn.addEventListener('click', async () => {
            if (deferredPrompt) {
                ...
                deferredPrompt = null;
            } else { window.location.href = '/feed/feed.html'; }
        });
    }
    if (closeBtn) { closeBtn.addEventListener('click', () => { pwaPopup.style.display = 'none'; }); }
});
...
function openVideo(videoId) {
    ...
    if (modal && target) {
        ...
        modal.classList.add('active'); lockScroll();
    }
}
function closeVideo() {
    const modal = document.getElementById(CONFIG.modalId);
    const target = document.getElementById(CONFIG.iframeTargetId);
    if (modal) { if (target) target.innerHTML = ''; modal.classList.remove('active'); unlockScroll(); }
}
...
if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        if (!privacyCheck || !privacyCheck.checked) { alert("Por favor, confirme que você leu e concorda com a Política de Privacidade."); return; }
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        closeVideo(); closeToast(); closePrivacyModal(); closeMobileMenu();
...
document.addEventListener('click', function(e) {
    if (e.target.id === CONFIG.modalId || (e.target.classList.contains('modal-overlay') && e.target.closest('#video-modal'))) closeVideo();
    if (e.target.id === CONFIG.toastContainerId || e.target.classList.contains('toast-close-btn')) closeToast();
    if (e.target.id === CONFIG.privacyModalId || (e.target.classList.contains('modal-overlay') && e.target.closest('#privacy-modal')) || e.target.classList.contains('modal-close')) closePrivacyModal();
    const translateDropdown = document.getElementById('translate-dropdown');
    const translateToggle = document.getElementById('translate-toggle');
    if (translateDropdown && translateToggle) { if (!translateDropdown.contains(e.target) && e.target !== translateToggle) translateDropdown.classList.remove('active'); }
});
...
function initCookieBanner() {
    ...
    if (!banner) return;
    if (!localStorage.getItem("vitaotub_cookies_accepted")) { setTimeout(() => { banner.classList.add("show"); }, 1000); }
    if (btnAccept) { btnAccept.onclick = function() { localStorage.setItem("vitaotub_cookies_accepted", "true"); banner.classList.remove("show"); }; }
    if (btnReject) { btnReject.onclick = function() { banner.classList.remove("show"); }; }
}
...
function toggleMobileMenu() {
    ...
    if (isActive) {
        menuToggle.classList.remove('active'); mobileMenu.classList.remove('active');
        menuToggle.setAttribute('aria-expanded', 'false'); menuToggle.setAttribute('aria-label', 'Abrir menu');
        document.body.style.overflow = '';
    } else {
        menuToggle.classList.add('active'); mobileMenu.classList.add('active');
        menuToggle.setAttribute('aria-expanded', 'true'); menuToggle.setAttribute('aria-label', 'Fechar menu');
    }
}
